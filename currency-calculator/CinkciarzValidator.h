...
vector<Profit> validDataToCurrencyObject(string dataToValid)
{
	Object currencyJSON;
	Array currencyArray;
	vector<Profit> currencyObjectsVector;
	...
	for(int i = 0; i < (int)currencyArray.size(); i++)
	{
		...
		currencyObjectsVector.push_back(Profit(symbol, fullName, rates));
	}
